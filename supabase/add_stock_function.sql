...
create or replace function calculate_max_stock(
  p_product_id text, -- Changed from uuid to text to support slugs
  p_variants jsonb,
...
declare
  ...
  max_quantity int;
begin
  -- 0. Resolve Product ID from Slug or UUID string
  -- Try to find by slug first which is our main use case
  ...
  if v_real_product_id is null then
    ...
    exception when others then
      -- Not a valid UUID and not a found slug
      return 9999;
    end;
  end if;
  ...
  with cart_consumption as (
    ...
    join products p on p.slug = (cart_item->>'productId') or p.id::text = (cart_item->>'productId') -- Match cart item to product to get recipe
