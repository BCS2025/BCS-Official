...
export default function OrderList({ items, products = [], onEdit, onDelete }) {
    if (items.length === 0) {
        return (
            <div className="text-center py-10 bg-store-50 rounded-lg border border-dashed border-bcs-border text-bcs-muted">
                尚未加入任何商品
            </div>
        );
    }
    ...
    const renderItemDetails = (item) => {
        ...
        return (
            <div className="text-xs text-bcs-muted space-y-1 mt-1">
                {product.fields.map(field => {
                    ...
                    return (
                        <div key={field.name}>
                            <span className="text-bcs-muted mr-1">{field.label}:</span>
    ...
    return (
        <div className="space-y-4">
            <div className="overflow-hidden rounded-lg border border-bcs-border bg-white">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-store-50 text-bcs-black font-medium">
                            <tr>
