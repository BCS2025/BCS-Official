...
export default function CustomQuote() {
    ...
    if (isSubmitted) {
        return (
            <div className="container mx-auto px-4 py-16">
                <div className="max-w-2xl mx-auto bg-white p-10 rounded-2xl shadow-sm border border-bcs-border text-center">
                    <CheckCircle className="mx-auto h-20 w-20 text-green-500 mb-6" />
                    <h2 className="text-3xl font-bold text-bcs-black mb-4">我們已收到您的需求！</h2>
                    <p className="text-lg text-bcs-black">我們的工程師將評估您的檔案與設計需求，並於 3 個工作天內為您提供正式報價與建議。</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <div className="max-w-3xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-bcs-border relative overflow-hidden">
                <h1 className="text-3xl font-bold text-bcs-black mb-8 text-center">鍛造工坊 - 專屬客製化評估</h1>
                    ...
                    {[1, 2, 3, 4].map((s) => (
                        <div key={s} className="flex flex-col items-center bg-white px-2">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${step === s ? 'bg-amber-600 text-white shadow-md ring-4 ring-amber-50' :
                                step > s ? 'bg-store-100 text-bcs-black' : 'bg-gray-100 text-gray-400'
                                }`}>
                                {step > s ? '✓' : s}
                            </div>
                            <span className={`text-xs mt-3 hidden sm:block font-medium ${step === s ? 'text-amber-800' : 'text-bcs-muted'}`}>
                                {s === 1 && '工程製造方式'}
                                {s === 2 && '結構材質規格'}
                                {s === 3 && '精密圖檔尺寸'}
                                {s === 4 && '聯絡顧問資訊'}
                ...
                {step === 1 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-bold text-bcs-black text-center mb-6">請選擇製造方式</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <button
                                onClick={() => handleMethodSelect('laser')}
                                className="border-2 border-bcs-border hover:border-amber-500 rounded-2xl p-8 flex flex-col items-center text-center transition-all hover:shadow-lg bg-store-50 hover:bg-white min-h-[220px] justify-center"
                            >
                                <Blocks className="w-16 h-16 text-amber-700 mb-4" />
                                <h3 className="text-xl font-bold text-bcs-black">雷射切割 / 雕刻</h3>
                                <p className="text-bcs-muted mt-2 text-sm leading-relaxed">Laser Cutting & Engraving<br />適用大面積木板、壓克力、不織布切割與快速表面打樣雕刻。</p>
                            </button>
                            <button
                            ...
                            >
                                ...
                                <h3 className="text-xl font-bold text-bcs-black">FDM 3D 列印</h3>
                                <p className="text-bcs-muted mt-2 text-sm leading-relaxed">3D Printing<br />適用於實心機構成型、工業產品原型開發設計與立體零件製造。</p>
                            </button>
                        </div>
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-bold text-bcs-black mb-6 pb-2 border-b">材質與規格選擇</h2>

                        {formData.method === 'laser' && (
                            <div className="space-y-6 bg-store-50 p-6 rounded-xl border border-bcs-border">
                                <div>
                                    <label className="block text-sm font-bold text-bcs-black mb-2">加工材質 (Material) *</label>
                                    <select
                                        className="w-full xl:w-2/3 p-3 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white"
                                ...
                                <div className="flex items-start gap-3 mt-4">
                                    ...
                                    />
                                    <label htmlFor="vectorService" className="text-bcs-black leading-relaxed max-w-xl">
                                        <span className="font-semibold text-bcs-black">需要向量繪圖/線條調整服務 (Requires vector line drawing/adjustment service)</span>
                        ...
                        {formData.method === '3dprint' && (
                            <div className="space-y-6 bg-store-50 p-6 rounded-xl border border-bcs-border">
                                <div>
                                    <label className="block text-sm font-bold text-bcs-black mb-2">工業線材 (Material) *</label>
                                    <select
                                        className="w-full xl:w-2/3 p-3 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white"
                                        value={formData.material}
                                        onChange={(e) => setFormData({ ...formData, material: e.target.value })}
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-bcs-black mb-2">內部填充率 (Infill Density) *</label>
                                    <select
                                        className="w-full xl:w-2/3 p-3 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white"
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-bcs-black mb-2">表面層高 (Layer Height) *</label>
                                    <select
                                        className="w-full xl:w-2/3 p-3 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white"
                        ...
                        <div className="flex justify-between mt-10">
                            <button
                                onClick={() => setStep(1)}
                                className="px-6 py-2.5 border border-bcs-border text-bcs-black font-medium rounded-full hover:bg-store-50 transition-colors flex items-center gap-2"
                            >
                ...
                {step === 3 && (
                    <div className="space-y-6">
                        <h2 className="text-xl font-bold text-bcs-black mb-6 pb-2 border-b">圖檔上傳與機具設定尺寸</h2>

                        <div>
                            <label className="block text-sm font-bold text-bcs-black mb-3">上傳檔案 (File Upload) *</label>
                            <div className="border-2 border-dashed border-bcs-border bg-store-50 rounded-xl p-10 text-center hover:bg-store-50 transition-colors relative">
                                <Upload className="mx-auto h-12 w-12 text-amber-600/70 mb-4" />
                                <div className="text-sm text-bcs-black font-medium mb-2">
                                    {formData.method === 'laser'
                                        ? '為確保切割順利，請提供 .ai, .dxf, .svg, 或 .pdf 格式'
                                        : '為確保順利轉檔切片，請提供 .stl, .step, .stp, 或 .obj 格式'}
                                </div>
                                <div className="text-xs text-bcs-muted mb-6 bg-store-100/50 inline-block px-3 py-1 rounded-full">最大檔案限制為 50MB</div>
                                <br />
                                <input
                                    type="file"
                                    id="fileUpload"
                                ...
                                <label
                                    htmlFor="fileUpload"
                                    className="cursor-pointer bg-white border border-bcs-border shadow-sm text-bcs-black font-bold px-6 py-2.5 rounded-lg hover:bg-amber-50 transition-colors inline-block"
                                >
                                    從本機選擇檔案
                                </label>
                                {formData.file && (
                        ...
                        <div className="pt-4">
                            <label className="block text-sm font-bold text-bcs-black mb-2">成品最大包絡尺寸 (Dimensions in millimeters) *</label>
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                <div>
                                    <label className="block text-xs font-bold text-bcs-muted mb-2 uppercase tracking-wide">X 軸 (Length)</label>
                                    <div className="relative">
                                        <input
                                            type="number"
                                            min="1"
                                            placeholder="例如: 150"
                                            className="w-full p-3 pr-10 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                                            value={formData.dimX}
                                            onChange={(e) => setFormData({ ...formData, dimX: e.target.value })}
                                        />
                                        <span className="absolute right-3 top-3 text-bcs-muted font-medium text-sm">mm</span>
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-bcs-muted mb-2 uppercase tracking-wide">Y 軸 (Width)</label>
                                    <div className="relative">
                                        <input
                                            type="number"
                                            min="1"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-bcs-muted mb-2 uppercase tracking-wide">Z 軸 (Height/Thickness)</label>
                                    <div className="relative">
                                        <input
                                            type="number"
                                            min="0"
                ...
                {step === 4 && (
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <h2 className="text-xl font-bold text-bcs-black mb-6 pb-2 border-b">聯絡人與最後設定</h2>
                        ...
                        <div className="bg-gradient-to-r from-amber-50 to-orange-50 p-6 rounded-xl border border-amber-200 mb-8 shadow-sm">
                            ...
                            <div className="flex items-start gap-4 bg-white p-4 rounded-lg">
                                ...
                                />
                                <label htmlFor="optimization" className="text-sm text-bcs-black cursor-pointer">
                                    <span className="font-bold text-base block mb-2 text-bcs-black">Require Mechanical Engineering Optimization? (需要專業機構優化建議)</span>
                                    <span className="text-bcs-muted leading-relaxed block">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                            <div>
                                <label className="block text-sm font-bold text-bcs-black mb-2">真實姓名 (Name) *</label>
                                <input
                                    type="text"
                                    required
                                    placeholder="以利我們稱呼您"
                                    className="w-full p-3 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                                    value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-bcs-black mb-2">電子郵件 (Email Address) *</label>
                                <input
                                    type="email"
                                    required
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold text-bcs-black mb-2">LINE ID (選填)</label>
                                <input
                                    type="text"
                                    className="w-full p-3 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                        ...
                        <div className="pt-2">
                            <label className="block text-sm font-bold text-bcs-black mb-2">客製細節備註 (Project specifics / Notes)</label>
                            <textarea
                                className="w-full p-4 border border-bcs-border rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 h-32 resize-none"
                        ...
                        <div className="flex flex-col-reverse sm:flex-row justify-between mt-10 pt-6 border-t border-bcs-border gap-4">
                            <button
                                type="button"
                                onClick={() => setStep(3)}
                                className="w-full sm:w-auto px-6 py-3 border border-bcs-border text-bcs-black font-medium rounded-full hover:bg-store-50 transition-colors flex justify-center items-center gap-2"
                            >
                            ...
                            <button
                                ...
                                className="w-full sm:w-auto px-8 py-3 bg-bcs-black text-white font-bold text-lg rounded-full hover:bg-black transition-all flex justify-center items-center gap-2 shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-bcs-muted group"
                            >
