...
function statusWebhookUrl() {
    ...
    const site = process.env.VITE_SITE_URL || 'https://bcs.tw';
    return `${site}/api/logistics/status-webhook`;
}
...
function buildC2CPayload(order, subType, receiverName, receiverCell, sender) {
    ...
    const payload = {
        ...
        ServerReplyURL: statusWebhookUrl(),
        IsCollection: 'N',
    };
...
function buildHomePayload(order, subType, receiverName, receiverCell, sender) {
    const userInfo = order.user_info || {};
...
export async function createLogisticsOrder(supabase, orderId) {
    ...
    const params = C2C_SUBTYPES.has(subType)
        ? buildC2CPayload(order, subType, receiverName, receiverCell, sender)
        : buildHomePayload(order, subType, receiverName, receiverCell, sender);
 ...
 */
export async function tryAutoCreateLogistics(supabase, orderId) {
    try {
        const { data: order, error } = await supabase
            .from('orders')
